...
IGNORE_PAGES = {'MediaWiki:', 'Predefiniç[ãa]o:', 'Wikcion[áa]rio:', 'Ajuda:'}
...
class WiktionaryHandler(xml.sax.ContentHandler):
    ...
    def get_synonyms(self):
        ...
        for line in self.temp_text:
            if re.match(REG_SYN_CATEGORY, line, re.IGNORECASE):
                values_flag = True
                group_number += 1
    ...
    def is_article(title):
        ...
        """
        if re.match(IGNORE_REG, title, re.IGNORECASE):
            return False
        else:
            return True
...
filename = 'ptwiktionary-20180301-pages-articles.xml'
